...
function I = plot_ellipse_in_image(I, v, ellipse_color, half_cross_len);
...
% ellipse_color = color of the ellipse
% half_cross_len = half of the cross length of the ellipse center
%
% Output:
...
if bit == 3
    ...
    if cy > half_cross_len & height - cy > half_cross_len & cx > half_cross_len & width - cx > half_cross_len
        I(cy,cx-half_cross_len:cx+half_cross_len,1) = ellipse_color(1);
        I(cy,cx-half_cross_len:cx+half_cross_len,2) = ellipse_color(2);
        I(cy,cx-half_cross_len:cx+half_cross_len,3) = ellipse_color(3);
        I(cy-half_cross_len:cy+half_cross_len, cx, 1) = ellipse_color(1);
        I(cy-half_cross_len:cy+half_cross_len, cx, 2) = ellipse_color(2);
        I(cy-half_cross_len:cy+half_cross_len, cx, 3) = ellipse_color(3);
    end
elseif bit == 1
    if ~exist('ellipse_color')
        ellipse_color = 255;
    ...
    % draw a cross to indication the ellipse center
    if cy > half_cross_len & height - cy > half_cross_len & cx > half_cross_len & width - cx > half_cross_len
        I(cy,cx-half_cross_len:cx+half_cross_len,1) = ellipse_color;
        I(cy-half_cross_len:cy+half_cross_len, cx, 1) = ellipse_color;
    end
end
