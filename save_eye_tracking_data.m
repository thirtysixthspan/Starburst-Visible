...
function save_eye_tracking_data(file_name, ellipse, scene)

% This function saves the eye tracking data including the parameters of
% pupil ellipse and the preditive gaze location in scene to a text file.
%
% Input:
...
fprintf(fid, 'index  pupil(a)  pupil(b)  pupil(cx)  pupil(cy)  pupil(theta) scene(x)  scene(y)\n');

for i=1:size(scene,1)
    fprintf(fid, '%5d  %8.2f  %8.2f  %9.2f  %9.2f  %12.2f %8.2f  %8.2f\n', ...
        i, ellipse(i,1), ellipse(i,2), ellipse(i,3), ellipse(i,4), ellipse(i,5), scene(i,1), scene(i,2));
end
fclose(fid);
