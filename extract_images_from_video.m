...
[sfname, spname] = uigetfile('*.mp4; *.mpg; *.mpeg; *.avi; *.wmv','Scene movie file');
sf = strcat(spname, sfname);

[efname, epname] = uigetfile('*.mp4; *.mpg; *.mpeg; *.avi; *.wmv','Eye movie file', spname);
ef = strcat(epname, efname);

eval(sprintf('!mkdir %s/Scene', spname));
eval(sprintf('!mkdir %s/Eye', epname));
eval(sprintf('!ffmpeg -i %s -r 30 -img jpeg %sScene/Scene_%%05d.jpg', sf, spname));
eval(sprintf('!ffmpeg -i %s -r 30 -img jpeg %sEye/Eye_%%05d.jpg', ef, epname));